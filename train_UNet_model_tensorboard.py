...
def build_graph():
    with tf.Graph().as_default() as g:
        ...
        model_dict = {'graph': g, 'inputs': [x_, y_],'Iou':iou,'Loss':loss, 'summary': summary}
    return model_dict
...
def train_model(model_dict, X_train, Y_train, max_epoch, max_iter, tsb_dir, restore=False, valid = True, ):
        ...
        with tf.Session() as sess:
            ...
            for epoch_i in range(max_epoch):
                ...
                for iter_i, train_batch in enumerate(train_zip):
                    ...
                    _,tr_iou,tr_loss,summary = sess.run([trainer, Iou_result, Loss_result,merged_summary], feed_dict = train_feed_dict)
                    train_writer.add_summary(summary, global_step = epoch_i * max_iter + iter_i)
...
def validate_model(model_dict, X_train, Y_train, restore):
    with model_dict['graph'].as_default():
        saver = tf.train.Saver(max_to_keep=0)
        with tf.Session() as sess:
            saver.restore(sess, restore)
...
if __name__ == "__main__":
    ...
    Y_train = np.load('C_train.npy')
    restore = False
    model_dict = build_graph() 
    train_model(model_dict, X_train, Y_train, 16, 500, './graph/', restore=False)